...
useEffect(() => {
  ...
  const pollAtsStatus = async (userId) => {
  let status = false;
  while (!status) {
    const res = await fetch(`http://localhost:5000/api/auth/profile/${userId}`);
    const data = await res.json();
    status = data.atsAnalyzed;
    setAtsAnalyzed(status);
    if (!status) await new Promise(r => setTimeout(r, 2000)); // wait 2s
  }
};
  ...
  try {
    ...
    if (data.extractedProfile) {
      setProfile((prev) => ({
        ...prev,
        name: data.extractedProfile.name || prev.name,
        phone: data.extractedProfile.phone || prev.phone,
        skills: data.extractedProfile.skills.length ? data.extractedProfile.skills : prev.skills,
      }));
    }
    ...
    showToast(err.message || "Failed to upload or analyze resume", "error");
  } finally {
    setIsAnalyzing(false);
  }
};
