...
const getStatusText = (status) => {
    switch (status?.toLowerCase()) {
        case "approved":
            return "Approved – HR will contact you soon for next steps";
        case "pending":
            return "Pending – Awaiting HR review";
        case "rejected":
            return "Rejected – Better luck next time";
        default:
            return status || "N/A";
    }
};
...
const userId = getUserId();
 ...
 const getStatusColor = (status) => {
    switch (status?.toLowerCase()) {
        case "approved":
            // Bright green for approved
            return "bg-emerald-100 text-emerald-800 border-emerald-200";
        case "accepted":
        case "hired":
            // Slightly softer green for accepted/hired
            return "bg-green-50 text-emerald-600 border-emerald-100";
        case "pending":
    ...
    <span className={`inline-flex items-center px-3 py-1.5 rounded-full text-[11px] font-bold border uppercase tracking-wider ${getStatusColor(app.status)}`}>
        ...
        {getStatusText(app.status)}
                                                ...
                                                {/* <td className="px-6 py-5">
                                                    <span className={`inline-flex items-center px-3 py-1.5 rounded-full text-[11px] font-bold border uppercase tracking-wider ${getStatusColor(app.status)}`}>
                                                        <Clock size={12} className="mr-1.5" />
                                                        {getStatusText(app.status)} {/* {app.status} */}
                                                    {/* </span>
                                                </td> */} 
