...
const { generateLearningResources } = require("../utils/aiLearningResources"); // NEW
...
router.post("/analyze-one", async (req, res) => {
  ...
  try {
    ...
    if (!mongoose.Types.ObjectId.isValid(userId) || !mongoose.Types.ObjectId.isValid(vacancyId)) {
    ...
    if (!user?.resume || !vacancy) {
      return res.status(400).json({ error: "User resume or vacancy not found" });
    }
    ...
    const learningResources = aiResult.missingSkills.length
      ? await generateLearningResources(aiResult.missingSkills)
      : {};
